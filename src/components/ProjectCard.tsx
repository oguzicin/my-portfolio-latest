  ...
  return (
    <div>
      <div className="w-[16rem] h-[16rem] bg-[#0e2443] rounded-xl gap-0 flex flex-col justify-evenly hover:-translate-y-2 duration-100 hover:bg-[#244773] custom-m:w-full custom-xs:w-[50rem] custom-xs:h-[10rem] custom-xm:w-full custom-sm:w-[13rem] custom-sm:h-[13rem]  ">
        <div className="h-fit  flex flex-row justify-left items-center gap-3 ms-4 mt-8 mb-4 ">
        ...
        <div className=" h-full ms-6 me-6 font-redditMono text-[0.75vw] text-left flex items-start justify-center text-gray-400 custom-xm:text-[0.7rem]">{text}
        </div>
        <div className="p-4 text-xl hover:text-[#64ffda] text-gray-300 w-full flex flex-col items-end justify-end h-fit custom-xm:text-[1.1rem]"><a href={link} target="_blank" rel="noopener noreferrer"><FaGithub /></a></div>
      </div>
    </div>
  );
};
